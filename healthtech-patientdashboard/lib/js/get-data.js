...
function getNutritionName(input = []) {
    ...
    if (input.excludeFoodModifier != null){
      nutrition_list2.innerText = input.excludeFoodModifier[0].text;
      nutrition_rec.innerText = "High need for MNT";
      return input.excludeFoodModifier[0].text;
      //var coding = input.excludeFoodModifier.coding.find(c => c.system === foodPref);
      //return coding && coding.display || "Unnamed Nutrition(TM)";
    }
    else if (input.foodPreferenceModifier != null){
      nutrition_list3.innerText = input.foodPreferenceModifier[0].text;
      nutrition_rec.innerText = "High need for MNT";
      return input.foodPreferenceModifier[0].text;
    }
    else if (input.oralDiet != null){
      nutrition_list4.innerText = input.oralDiet[0].instruction;
      nutrition_rec.innerText = "High need for MNT";
      return input.oralDiet[0].text;
    }
    else {
      /*test = JSON.stringify(input.excludeFoodModifier);
      nutrition_list2.innerText = input.excludeFoodModifier[0].id;
      nutrition_list3.innerText = input.excludeFoodModifier[0].text;
      nutrition_list4.innerText = test[0].id;*/
      
      //test.length; WORKS = 200
      return;
    }
    //var coding = input.excludeFoodModifier.coding.find(c => c.system === foodPref);
    //return coding && coding.display || "Unnamed Nutrition(TM)";
